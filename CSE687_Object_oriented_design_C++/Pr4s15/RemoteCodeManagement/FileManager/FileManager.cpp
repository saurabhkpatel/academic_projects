...
//<-------getfull Spec file path---->
std::string FileManager::getFullFilePathSpec(std::string path)
{
	return FileSystem::Path::getFullFileSpec(path);
}
...
{
	std::string temp = getFullFilePathSpec(".");
...
{
	DataStore ds;
	std::vector<std::string> patt{ "*.cpp","*.h"};
